            ...
            if mi_formulario.is_valid():
                ...
                if verbo[2]=='Correcto':
                    ...
                    if df_presente_simple.shape[1]-1 == 0: #Fin del Juego
                        ...
                        return render(request, 'italiano/respuesta_correcta.html',{'dato':datos,'significado':verbo[1]})
                else:
                    Juego.incorrectas += 1
                    Juego.listado_incorrectas.append(verbo)
                    return render(request, 'italiano/respuesta_incorrecta.html',{'dato':datos,'significado':verbo[1]})
...
class Conjugaciones(TemplateView):
    ...
    def formulario_conjugacion(request):
        ...
        if request.method == "POST":
            ...
            if mi_formulario.is_valid():
                datos = mi_formulario.cleaned_data
                verbo_conjugado_presente = Conjugacion_presente_semplice.objects.using('italiano_db').filter(verbo=datos['verbo'])
                verbo_conjugado_pasado = Conjugacion_passato_prossimo.objects.using('italiano_db').filter(verbo=datos['verbo'])
                verbo_conjugado_futuro = Conjugacion_futuro_semplice.objects.using('italiano_db').filter(verbo=datos['verbo'])


                return render(request, 'italiano/conjugaciones.html',{'dato_presente_semplice':verbo_conjugado_presente,
